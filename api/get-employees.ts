...
if (isVercel) {
  ...
  if (requiredEnvVars.PG_HOST &&
    ...
    try {
      ...
      console.log('✅ Remote DB pool created for get-employees on Vercel');
    } catch (error) {
      console.warn('⚠️ Could not create remote DB pool:', error);
      pool = null;
    }
  } else {
    console.log('🌐 Running on Vercel - get-employees will return empty data (no remote DB configured)');
  }
} else {
  const requiredEnvVars = {
  ...
  try {
    ...
    console.log('✅ Local DB pool created for get-employees');
  } catch (error) {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // On Vercel: return empty data (employees come from management_data.json via read-json-data)
  if (isVercel || !pool) {
    console.log('⚠️ get-employees: No DB connection, returning empty array (employees should come from JSON)');
    return res.status(200).json({
      success: true,
      employees: [],
      count: 0,
      note: 'Employees are loaded from management_data.json via /api/read-json-data',
    });
  }
