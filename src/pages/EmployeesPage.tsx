...
const EmployeesPage: React.FC<EmployeesPageProps> = ({
  ...
  // Fetch employees if not provided (User request: ensure join works by fetching missing list)
  React.useEffect(() => {
    if ((!allEmployees || allEmployees.length === 0) && fetchedEmployees.length === 0) {
      const fetchEmployees = async () => {
        // Dynamically import to avoid circular dependencies if any, or just strictly typed
        const { getEmployees } = await import('@/data/dataProvider');
        console.log('🔄 EmployeesPage: Fetching missing employee list...');
        const list = await getEmployees();
        if (list.length > 0) {
          setFetchedEmployees(list as Employee[]);
        }
      };
      fetchEmployees();
    }
  }, [allEmployees, fetchedEmployees.length]);

  // Use passed employees or fetched ones
  const effectiveAllEmployees = allEmployees && allEmployees.length > 0 ? allEmployees : fetchedEmployees;
  ...
  const filteredEmployeeSummary = useMemo(() => {
    ...
    const derivedFromEmployees = effectiveAllEmployees.find(e =>
  ...
  }, [employeeSummary, searchTerm, profile, effectiveAllEmployees]);
