...
const aggregateRangeForStores = (
    ...
    metrics.forEach(metric => {
        ...
        const storeMatch = allowedStores.has(metric.store);
        const idMatch = allowedStoreIds && (metric as any).storeId && allowedStoreIds.has((metric as any).storeId);

        if (!storeMatch && !idMatch) return;

        // Use consistent store name if we matched by ID but name differs (optional, but keep simple for now)
        // For the key, we use metric.store as is, assuming it's the canonical name in most cases
        const key = metric.store;

        const existing = result.get(key) || { sales: 0, visitors: 0, transactions: 0 };
...
export const useDataProcessing = ({
    ...
    const roleFilteredData = useMemo(() => {
        ...
        const visibleStoreIds = new Set(visibleStores.filter(s => s.id).map(s => s.id));
        ...
        const filterMetrics = (m: DailyMetric) => visibleStoreNames.has(m.store) || ((m as any).storeId && visibleStoreIds.has((m as any).storeId));
        const filterSales = (s: SalesTransaction) => visibleStoreNames.has(s['Outlet Name']) || ((s as any)['Store Code'] && visibleStoreIds.has((s as any)['Store Code']));
        ...
        return {
            ...
            visibleStoreIds, // Expose this for later use
        };
    ...
    const dateFilteredData = useMemo(() => {
        const filterByDate = (item: DailyMetric | SalesTransaction) => {
            const itemDateValue = 'date' in item ? item.date : (item as any)['Bill Dt.'];
    ...
    const areaFilteredData = useMemo(() => {
        const filteredStores = roleFilteredData.stores.filter(store =>
            ...
            (areaStoreFilter.store === 'All' || store.id === areaStoreFilter.store || store.name === areaStoreFilter.store) &&
            (areaStoreFilter.city === 'All' || !areaStoreFilter.city || store.city === areaStoreFilter.city)
        );
        const storeIds = new Set<string>();
        filteredStores.forEach(s => {
            if (s.id) storeIds.add(String(s.id).trim());
            if (s.name) storeIds.add(String(s.name).trim());
            if ((s as any).store_id) storeIds.add(String((s as any).store_id).trim());
        });

        // Scoping employees to selected stores (matches by ID primarily)
        let employeesInScope = roleFilteredData.employees.filter(e => {
            const empStore = String(getEmployeeStoreForPeriod(e, dateFilter) || '').trim();
            return !empStore || storeIds.has(empStore);
        });

        // Fallback: If area filter is "All" but employees were somehow filtered out, 
        // keep them to prevent empty states when data mapping is incomplete.
        if (employeesInScope.length === 0 && areaStoreFilter.store === 'All' && areaStoreFilter.areaManager === 'All') {
            employeesInScope = roleFilteredData.employees;
        }
        ...
        if (profile?.role === 'store_manager') {
            ...
            const effectiveStoreId = String((profile as any).storeId || profile.store || derivedStoreFromEmployees?.currentStore || '').trim();
            employeesInScope = roleFilteredData.employees.filter(e => {
                const empStore = String(getEmployeeStoreForPeriod(e, dateFilter) || '').trim();
                return empStore === effectiveStoreId;
            });
        }
        ...
        return {
            ...
            metrics: dateFilteredData.metrics.filter(m => storeIds.has(String(m.store).trim())),
            sales: dateFilteredData.sales.filter(s => storeIds.has(String(s['Outlet Name'] || (s as any).outlet_id || '').trim())),
            duvetSales: dateFilteredData.duvetSales.filter(s => storeIds.has(String(s['Outlet Name'] || (s as any).outlet_id || '').trim())),
        };
    ...
    const storeIdsSet = useMemo(() => new Set(areaFilteredData.stores.filter(s => s.id).map(store => store.id)), [areaFilteredData.stores]);
    ...
    const employeeSummary = useMemo(() => {
        ...
        areaFilteredData.employees.forEach(employee => {
            const storeKey = getEmployeeStoreForPeriod(employee, dateFilter);

            // Critical Fix: Find the canonical store name for grouping, supporting both ID and Name
            const canonicalStore = areaFilteredData.stores.find(s =>
                s.name === storeKey || String(s.id) === String(storeKey) || String((s as any).store_id) === String(storeKey)
            );

            if (!canonicalStore) return;
            const storeName = canonicalStore.name;

            // Try to match by employeeId first, then by name as fallback
            const metricsForEmployee = areaFilteredData.metrics.filter(m =>
                (m.employeeId && m.employeeId === employee.employeeId) ||
                (m.employee && m.employee === employee.name)
            );
            const totalSales = metricsForEmployee.reduce((sum, m) => sum + (m.totalSales || 0), 0);
            const totalTransactions = metricsForEmployee.reduce((sum, m) => sum + (m.transactionCount || 0), 0);
    ...
    const duvetSummary = useMemo((): DuvetSummary => {
        ...
        return areaFilteredData.duvetSales.reduce((acc, sale) => {
            ...
            if (category) {
                ...
                acc[storeName][category] = (Number(acc[storeName][category]) || 0) + sale['Sold Qty'];
